...
import { hasGodotCacheDir } from "../godot/cache.js";
 ...
 * `runGodotImport`'s doc comment) - success is instead judged by whether
 * `hasGodotCacheDir` reports a `.godot/` cache now exists for project_path.
 */
function importResultToToolResult(
  result: RunGodotImportResult,
  projectPath: string,
  deps: Pick<ProjectToolsDeps, "hasGodotCacheDir">,
) {
  switch (result.kind) {
    case "completed": {
