...
- **Cap (new integration case, the key test):** write `godot_mcp/network/deferred_op_timeout_ms = 1` into the fixture project's `project.godot` **before** the editor launches (settings persisted on disk pre-launch — no debounced-save race, per the #96 lesson). Call `import_assets`: the deadline check runs before the task's first tick, and that tick arrives no earlier than the next editor frame (> 1 ms after arming), so expiry fires deterministically — even an instantly-finishing scan cannot complete first (completion needs at least two ticks via the observed path, or ten via grace). A larger cap (e.g. 100 ms) would race a fast fixture scan that gets observed and finishes inside the window. Assert the structured `deferred_op_timeout` error, then issue a follow-up op and assert it succeeds — the queue drained, no permanent wedge (REQ-A-12 acceptance).
