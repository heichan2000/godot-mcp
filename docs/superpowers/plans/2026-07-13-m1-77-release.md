...
describe("renderToolDocs", () => {
  ...
  it("renders _none_ for a parameterless tool", () => {
    ...
    expect(renderToolDocs([group])).toContain("| `demo_ping_nothing` | Ping with no parameters. | _none_ |");
  });
...
export function buildToolGroups(deps: ServerDeps): ToolGroup[] {
  return [
    ...
    {
      title: "Diagnostics",
      tools: createDiagnosticsTools({ bridge: deps.bridge, lspPort: deps.lspPort ?? DEFAULT_LSP_PORT }),
    },
    {
      title: "Run & debug",
...
const outputPath = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "docs", "tools.md");
...
describe.runIf(hasGodot)("M1 exit demo: the §11 smoke loop walking all 19 §6.6 rows", () => {
  ...
  it("walked all 19 §6.6 parity rows through the addon", () => {
    ...
    for (const entry of PARITY_ROWS) {
      expect(registered.has(entry.tool), `row ${entry.row}: ${entry.tool} not registered`).toBe(true);
    }
    ...
    expect(
      missing.map((entry) => `row ${entry.row} (${entry.legacy} -> ${entry.tool})`),
    ).toEqual([]);
